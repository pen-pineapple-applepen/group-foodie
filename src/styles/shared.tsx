...
const SideBarMenu = ({ sideBarOpen, toggleMenu }: SideBarMenuProps) => {
  ...
  const { logOut, setCurrentUser, updateCurrentGroup, purgePaymentsList } = allActions;
  ...
  return (
    <>
      ...
      <OrangeNavbarContainer className="is-fixed-top" active={active}>
        <NavbarBrand>
          {needBackArrow && <BackArrow onClick={handleBackArrowClick} />}
          <NavbarItem>
            <GroupFoodieLogo onClick={handleLogoClick}>Group Foodie</GroupFoodieLogo>
          </NavbarItem>

          {hasBurger && loggedIn && (
            <NavbarBurger onClick={() => setSideBarOpen((prev) => !prev)} />
          )}
        </NavbarBrand>
...
export const ProfileImage: (props: profileImageProps) => JSX.Element = ({ src, size }) => {
  return <SizedImage src={src} rounded={true} fullwidth={false} size={size || 64} />;
};
